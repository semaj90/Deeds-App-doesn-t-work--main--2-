    ...
    async function handleFileUpload() {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                const fileArray = new Uint8Array(await files[i].arrayBuffer());
                const result = await invoke('upload_evidence_file', {
                    filename: files[i].name,
                    fileData: Array.from(fileArray),
                    fileType: files[i].type
                });
                filePaths.push(result as string);
            }

            uploadMessage = `Files uploaded successfully! ${filePaths.length} files processed.`;
...
<svelte:head>
    <title>Dashboard - Deeds App</title>
</svelte:head>
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h2 class="mb-2">
                                ...
                                Welcome back, {$session?.user?.name || 'Prosecutor'}!
                            </h2>
