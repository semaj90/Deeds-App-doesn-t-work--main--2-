# Multimodal Legal NLP/LLM Setup Script (Fixed)
...
if (-not $SkipDocker) {
    ...
    try {
        ...
        if ($dockerVersion) {
            ...
            Write-Host "⚠️ Docker not found. Please install Docker to use Qdrant" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "⚠️ Docker not available. Qdrant setup skipped" -ForegroundColor Yellow
    }
}
...
Write-Host "🗃️ Step 5: Setting up Database" -ForegroundColor Yellow
