...
export class PredictiveAnalyzer {
  ...
  private async findRelatedCases(description: string, entities: ExtractedEntity[], excludeCaseId?: string): Promise<RelatedCase[]> {
    try {
      ...
      if (excludeCaseId) {
        whereConditions.push(sql`${cases.id} != ${excludeCaseId}`);
      }
  ...
  private async cacheAnalysis(contentHash: string, originalText: string, analysis: CaseAnalysis): Promise<void> {
    try {      await db.insert(nlpAnalysisCache).values({
        contentHash,
        contentType: 'case_description',
        originalText,
        analysis: analysis,
        entities: analysis.entities,
        sentiment: analysis.sentiment ? analysis.sentiment.toString() : null,
        confidence: analysis.confidence ? analysis.confidence.toString() : null,
  ...
  async getSavedStatements(category?: string, userId?: string): Promise<any[]> {
    try {
      ...
      if (category) {
        whereConditions.push(eq(savedStatements.category, category));
      }
