...
export async function GET({ params }) {
    const { id } = params;
    const evidenceItem = await db.select().from(evidence).where(eq(evidence.id, id)).limit(1);
...
export async function DELETE({ params, locals }) {
    ...
    const { id } = params;
    const deleted = await db.delete(evidence).where(eq(evidence.id, id)).returning();
    if (!deleted.length) {
