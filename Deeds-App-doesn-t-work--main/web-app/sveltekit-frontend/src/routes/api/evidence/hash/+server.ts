...
import { evidence, cases, users } from '$lib/server/db/unified-schema';
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  if (!/^[a-f0-9]{64}$/i.test(hash)) {
    return json({ error: 'Invalid hash format. Expected 64-character hexadecimal string (SHA256)' }, { status: 400 });
  }
