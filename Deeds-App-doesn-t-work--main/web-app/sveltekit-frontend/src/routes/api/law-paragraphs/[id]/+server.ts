...
export async function GET({ params }) {
    const { id } = params;
    const lawParagraph = await db.select().from(lawParagraphs).where(eq(lawParagraphs.id, id)).limit(1);
...
export async function DELETE({ params }) {
    ...
    const deletedLawParagraph = await db.delete(lawParagraphs).where(eq(lawParagraphs.id, id)).returning();
