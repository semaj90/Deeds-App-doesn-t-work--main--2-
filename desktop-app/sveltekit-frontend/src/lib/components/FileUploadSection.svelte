<script lang="ts">
	...
	import { loki, lokiStore } from '$lib/stores/lokiStore';
	...
	>
		{#if uploads.length === 0}
			<div class="drop-zone-content">
				<UploadSimple class="upload-icon" size={48} />
		...
		{:else}
			<div class="upload-summary">
				...
				<button type="button" class="btn-link" on:click|stopPropagation={triggerFileSelect}>
					Add more
				</button>
			</div>
		{/if}
				...
				<div class="file-item" class:uploading={upload.status === 'uploading'}>
					<div class="file-info">
						...
						<div class="file-status">
							...
							{:else if upload.status === 'error'}
								<WarningCircle size={20} class="status-icon error" />
							{/if}
						</div>
