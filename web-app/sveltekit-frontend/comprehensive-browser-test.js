...
async function testUserProfile() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log(`   📋 Name: ${data.user?.name}, Email: ${data.user?.email}, Role: ${data.user?.role}`);
        } else {
...
async function testReportManagement() {
    ...
    try {
        const reportData = {
            ...
            `,
            summary: 'Comprehensive browser test report documenting system functionality and test results.',
            caseId: testCaseId,
...
async function testCitationPoints() {
    ...
    try {
        const citationData = {
            ...
            page: 27,
            context: 'This precedent establishes the four-factor test for evaluating speedy trial violations: length of delay, reason for delay, defendant\'s assertion of right, and prejudice to defendant.',
