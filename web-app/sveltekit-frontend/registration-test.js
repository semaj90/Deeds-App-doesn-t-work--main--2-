...
try {
    const registerServerContent = await fs.readFile('src/routes/register/+page.server.ts', 'utf-8');
    if (registerServerContent.includes('export const actions') && registerServerContent.includes('fail') && registerServerContent.includes('redirect')) {
...
try {
    const registerPageContent = await fs.readFile('src/routes/register/+page.svelte', 'utf-8');
    if (registerPageContent.includes('use:enhance') && registerPageContent.includes('form?.error') && registerPageContent.includes('bind:value')) {
...
try {
    const loginPageContent = await fs.readFile('src/routes/login/+page.svelte', 'utf-8');
    if (loginPageContent.includes('showRegistrationSuccess') && loginPageContent.includes('success-alert')) {
...
// Check CanvasEditor
try {
    const canvasEditorContent = await fs.readFile('src/lib/components/CanvasEditor.svelte', 'utf-8');
...
// Check Avatar
try {
    const avatarContent = await fs.readFile('src/lib/components/Avatar.svelte', 'utf-8');
    if (avatarContent.includes('on:keydown') && avatarContent.includes('aria-label') && avatarContent.includes('.clickable .avatar:hover')) {
