...
async function initializeVectorSearch() {
  ...
  if (!isPostgreSQL()) {
    console.log('⚠️  Vector search requires PostgreSQL. Current environment uses SQLite.');
    console.log('💡 Switch to testing or production environment to use vector search.');
    return;
  }
  ...
  try {
    ...
    if (qdrantHealthy) {
      ...
      console.log('⚠️  Qdrant is not available. Vector search will use pgvector only.');
    }
    ...
    try {
      ...
      console.log('ℹ️  Vector indexes may already exist or pgvector extension is not installed');
      console.log('   This is normal if running for the first time or in development mode');
    }
    ...
    const { generateEmbedding } = await import('$lib/server/ai/embeddings-simple');
