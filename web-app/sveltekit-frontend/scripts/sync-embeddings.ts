...
async function syncEmbeddings(
  ...
  try {
    ...
    for (let i = 0; i < documents.length; i += batchSize) {
      ...
      for (const doc of batch) {
        try {
          ...
          switch (type) {
            case 'cases':
              textContent = `${doc.title || ''} ${doc.description || ''} ${doc.notes || ''}`.trim();
              break;
            case 'evidence':
              textContent = `${doc.title || ''} ${doc.description || ''} ${doc.content || ''}`.trim();
              break;
            case 'documents':
    ...
    console.log(`   ✅ Completed: ${stats.updated} updated, ${stats.errors} errors, ${stats.skipped} skipped`);
    return stats;
...
async function main() {
  ...
  try {
    ...
    if (!isPostgreSQL()) {
      console.log('⚠️  SQLite detected - vector embeddings will not be stored in database');
    ...
    if (qdrantHealthy) {
      ...
      console.log('⚠️  Qdrant not available - embeddings will only be stored in PostgreSQL');
    }
    ...
    const syncType = args.find(arg => ['cases', 'evidence', 'documents', 'all'].includes(arg)) as 'cases' | 'evidence' | 'documents' | 'all' || 'all';
    ...
    console.log('  • Verify embedding service is working (OpenAI API key, etc.)');
    process.exit(1);
  }
}
