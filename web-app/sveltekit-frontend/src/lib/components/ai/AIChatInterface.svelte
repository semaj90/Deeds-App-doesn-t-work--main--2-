...
<div class="ai-chat-interface {className}" style="max-height: {maxHeight}">
  <!-- Status Bar -->
  ...
  >
    ...
    {:else if $conversation.messages.length === 0}
      <div class="welcome-message">
        ...
        <p class="note">
          <strong>Note:</strong> This AI provides general information only and does not constitute legal advice.
        </p>
      </div>
    {:else}
      {#each $conversation.messages as message (message.id)}
        <AIChatMessage 
          {message}
          showSources={message.role === 'assistant' && message.sources && message.sources.length > 0}
