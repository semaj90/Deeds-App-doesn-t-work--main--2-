	...
	<div class={inputContainerClasses}>
		...
		{#if icon && iconPosition === 'left'}
			<div class={iconClasses}>
				<iconify-icon icon="ph:{icon}" class="w-4 h-4"></iconify-icon>
			</div>
		{/if}
		...
		{:else if success}
			<div class="absolute right-3 flex items-center">
				<iconify-icon icon="ph:check-circle" class="w-4 h-4 text-green-500"></iconify-icon>
			</div>
		{:else if hasError}
			<div class="absolute right-3 flex items-center">
				<iconify-icon icon="ph:warning-circle" class="w-4 h-4 text-red-500"></iconify-icon>
			</div>
		...
		{#if showClearButton}
			<button
				type="button"
				class="absolute right-3 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
				on:click={handleClear}
				tabindex="-1"
				aria-label="Clear input"
			>
				<iconify-icon icon="ph:x" class="w-3 h-3 text-gray-400"></iconify-icon>
			</button>
		{/if}
	</div>
