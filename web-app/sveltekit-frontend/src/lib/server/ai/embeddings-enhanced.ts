...
interface EnhancedEmbeddingOptions {
  provider?: 'auto' | 'local' | 'openai' | 'legal-bert';
  cache?: boolean;
  maxTokens?: number;
  legalDomain?: boolean;
  batchSize?: number;
...
export async function generateEnhancedEmbedding(
  ...
  try {
    ...
    if (cache && !isArray && Array.isArray(result) && !Array.isArray(result[0])) {
    ...
    if (provider === 'auto' || provider === 'local' || provider === 'legal-bert') {
...
export async function calculateLegalSimilarity(
  ...
  if (aiService.getStatus().tauriAvailable) {
    // Use local legal similarity calculation
    return aiService.getLegalSimilarity(doc1, doc2);
  } else {
