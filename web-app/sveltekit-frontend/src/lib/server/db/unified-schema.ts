...
export const cases = pgTable('cases', {
  ...
  titleEmbeddingIdx: index('cases_title_embedding_idx').using('hnsw', table.titleEmbedding.op('vector_cosine_ops')),
  descriptionEmbeddingIdx: index('cases_description_embedding_idx').using('hnsw', table.descriptionEmbedding.op('vector_cosine_ops')),
  fullTextEmbeddingIdx: index('cases_fulltext_embedding_idx').using('hnsw', table.fullTextEmbedding.op('vector_cosine_ops')),
...
export const evidence = pgTable('evidence', {
  ...
  summaryEmbeddingIdx: index('evidence_summary_embedding_idx').using('hnsw', table.summaryEmbedding.op('vector_cosine_ops')),
...
export const reports = pgTable('reports', {
  ...
  caseId: uuid('case_id').references(() => cases.id, { onDelete: 'cascade' }),
  reportType: varchar('report_type', { length: 50 }).default('prosecution_memo').notNull(),
...
export const canvasStates = pgTable('canvas_states', {
  ...
  dimensions: jsonb('dimensions').default({ width: 800, height: 600 }).notNull(),
...
export const citationPoints = pgTable('citation_points', {
  ...
  aiSummary: text('ai_summary'),
  relevanceScore: decimal('relevance_score', { precision: 4, scale: 3 }).default('0.0'),
  metadata: jsonb('metadata').default({}).notNull(),
...
export const hashVerifications = pgTable('hash_verifications', {
  ...
  verificationMethod: varchar('verification_method', { length: 50 }).default('manual').notNull(),
