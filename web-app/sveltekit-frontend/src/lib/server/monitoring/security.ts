  ...
  rateLimit(type: keyof SecurityConfig['rateLimits'] = 'general') {
    return (event: RequestEvent) => {
      ...
      if (this.blockedIPs.has(ip)) {
        logWarn('Blocked IP attempted request', { ip, endpoint: event.url.pathname });
      ...
      if (entry.count > limit.requests) {
        ...
        throw error(429, `Rate limit exceeded. Retry after ${Math.ceil((entry.resetTime - now) / 1000)} seconds`);
      }
