...
class EnhancedAIService {
      ...
      switch (provider) {
        ...
        case 'tauri-bert':
          if (tauriLLM.isAvailable()) {
            ...
            result = await tauriLLM.generateEmbedding(inputs, options) as number[][];
          } else {
  ...
  async generateResponse(
    ...
    const fullPrompt = this.buildFullPrompt(prompt, systemPrompt, options.context);
  ...
  async analyzeLegalDocument(text: string): Promise<{
    ...
    try {
      ...
      const [classification, summary] = await Promise.all([
        tauriLLM.classifyLegalDocument(text),
  ...
  async batchAnalyzeDocuments(
    ...
    if (tauriLLM.isAvailable()) {
      // Use local batch processing for better performance
      const results = await tauriLLM.batchProcessDocuments(
        documents,
        ['embed', 'classify', 'summarize']
      );
      
      // Ensure embedding is always present
      ...
      for (const doc of documents) {
        try {
          const embedding = await this.generateEmbedding(doc.text, { legalDomain: true }) as number[];
  ...
  private selectEmbeddingProvider(options: EmbeddingOptions): EmbeddingProvider {
    ...
    if (this.config.preferLocal && this.config.legalDomain && tauriLLM.isAvailable()) {
  ...
  private selectLLMProvider(options: GenerationOptions): LLMProvider {
    ...
    if (this.config.preferLocal && options.legalContext && tauriLLM.isAvailable()) {
      const models = tauriLLM.getAvailableModels();
