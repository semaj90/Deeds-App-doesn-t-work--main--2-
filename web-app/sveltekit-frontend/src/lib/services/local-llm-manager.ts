...
export const localLLMConfig: LocalLLMConfig = {
  ...
  paths: {
    ...
    configFile: dev ? './llm-config.json' : '%LOCALAPPDATA%/LegalRAG/config.json'
  },
  development: {
...
class LocalLLMManager {
  ...
  async checkModelAvailability(): Promise<{
    ...
    try {
      if (typeof window !== 'undefined' && window.__TAURI__) {
        ...
        availability.classification = models.some(m => m.type === 'classification');
      } else {
  ...
  async downloadModel(modelType: 'embedding' | 'chat' | 'classification'): Promise<boolean> {
    try {
  ...
  async getOptimalProvider(queryType: 'embedding' | 'chat' | 'classification'): Promise<'local' | 'cloud'> {
  ...
  async getSystemInfo(): Promise<{
    ...
    try {
      if (systemInfo.isDesktop) {
        ...
        const info = await fetch('http://localhost:3000/api/system/info').then(r => r.json());
        Object.assign(systemInfo, info);
      } else {
        // Web environment - limited system info
        systemInfo.availableMemory = (navigator as any).deviceMemory * 1024 || 4096; // MB
