...
try {
  const aiServiceModule = await import('$lib/services/ai-service');
...
try {
  const cacheModule = await import('$lib/server/cache/redis');
...
try {
  const tauriModule = await import('$lib/services/tauri-llm');
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const contextText = relevantSources
      .map((source: any) => `Title: ${source.title}\nContent: ${source.content}\nType: ${source.type}`)
      .join('\n\n---\n\n');
...
- Be concise but thorough`;
        ...
        provider = 'local';
        model = response.model || 'gemma2:2b';
