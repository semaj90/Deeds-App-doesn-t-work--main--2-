...
export const POST: RequestHandler = async ({ request, locals }) => {
	try {
		...
		if (!canvasState || !reportId) {
			return json({ error: 'Canvas state and report ID are required' }, { status: 400 });
		}
...
export const GET: RequestHandler = async ({ url, locals }) => {
	try {
		...
		if (!canvasId && !reportId) {
			return json({ error: 'Canvas ID or Report ID is required' }, { status: 400 });
		}
...
export const DELETE: RequestHandler = async ({ request, locals }) => {
	try {
		...
		if (canvasState.updatedBy !== locals.user?.id && locals.user?.role !== 'admin') {
