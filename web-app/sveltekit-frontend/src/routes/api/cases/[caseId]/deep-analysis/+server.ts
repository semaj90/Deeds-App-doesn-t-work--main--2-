...
import { cases, evidence, caseActivities } from '$lib/server/db/schema';
...
export const POST: RequestHandler = async ({ params, locals, request }) => {
	...
	try {
		...
		const [qdrantFragmentResults, qdrantEvidenceResults] = await Promise.allSettled([
		...
		const relevantFragments = qdrantFragmentResults.status === 'fulfilled' 
			? qdrantFragmentResults.value.map((hit) => hit.payload?.content).join('\n\n')
			: '';

		const relevantEvidenceSummaries = qdrantEvidenceResults.status === 'fulfilled'
			? qdrantEvidenceResults.value.map((hit) => hit.payload?.aiSummary).join('\n\n')
			: '';
...
`.trim();
		...
		promises.push(
			...
			}).then((res) => res.json().then((data) => ({ source: 'firm_ai', data, ok: res.ok })))
		);
		...
		if (enableMultiLLM && OPENAI_API_KEY) {
			promises.push(
				...
				}).then((res) => res.json().then((data) => ({ source: 'openai', data, ok: res.ok })))
			);
		}
			...
			if (result.status === 'fulfilled' && result.value.ok) {
				...
				if (source === 'firm_ai') {
					try {
						...
						analysisResults.firm_ai = { output: parsedResponse, source: 'Local LLM (JSON)' };
					} catch (e) {
						// Fallback if JSON parsing fails despite the grammar (very unlikely)
						console.error("Local LLM output was not valid JSON:", data.response);
						analysisResults.firm_ai = { output: data.response, source: 'Local LLM (Raw)' };
					}
				} else if (source === 'openai') {
					analysisResults.openai = { output: data.choices[0].message.content, source: 'OpenAI API' };
				}
