...
import { cases, criminals, evidence, statutes, caseActivities } from '$lib/server/db/schema';
import { QdrantClient } from '@qdrant/js-client-rest';
import { eq, ilike, sql } from 'drizzle-orm';
import { env } from '$env/dynamic/private';
import type { RequestHandler } from '@sveltejs/kit';

const qdrantClient = new QdrantClient({ host: env.QDRANT_URL || 'localhost', port: 6333 });
...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	try {
		...
		const evidenceCount = await db.select({ count: sql<number>`count(*)` }).from(evidence).where(eq(evidence.caseId, caseId));
		...
		const activitiesCount = await db.select({ count: sql<number>`count(*)` }).from(caseActivities).where(eq(caseActivities.caseId, caseId));
		if (currentCase.status === 'open' && activitiesCount[0]?.count === 0) {
			recommendations.push({
				id: 'rec-first-activity',
				type: 'task',
				title: 'Schedule First Activity',
				description: 'This case is open but has no activities. Consider scheduling an initial review or evidence collection.',
				confidence: 0.8,
			...
			if (nlpResponse.ok) {
				...
				for (const hit of searchResult) {
					recommendations.push({
						...
						actionData: { caseId: hit.id, title: hit.payload?.title, summary: hit.payload?.aiSummary }
					});
				}
			}
		} catch (vectorError) {
		...
		return json({ success: true, recommendations: recommendations.sort((a, b) => b.confidence - a.confidence).slice(0, 10) });
	} catch (error) {
