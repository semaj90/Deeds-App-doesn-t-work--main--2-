...
import { generateEmbedding, generateBatchEmbeddings } from '$lib/server/ai/embeddings-simple';
...
export const PATCH: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    switch (type) {
      case 'cases':
        syncResult = await syncDocumentEmbeddings('cases', limit, forceRegenerate);
        break;
      case 'evidence':
        syncResult = await syncDocumentEmbeddings('evidence', limit, forceRegenerate);
        break;
      case 'documents':
        syncResult = await syncDocumentEmbeddings('documents', limit, forceRegenerate);
        break;
      case 'all':
        const casesResult = await syncDocumentEmbeddings('cases', limit, forceRegenerate);
        const evidenceResult = await syncDocumentEmbeddings('evidence', limit, forceRegenerate);
        const documentsResult = await syncDocumentEmbeddings('documents', limit, forceRegenerate);
        
        syncResult = {
          total: casesResult.total + evidenceResult.total + documentsResult.total,
          updated: casesResult.updated + evidenceResult.updated + documentsResult.updated,
          errors: casesResult.errors + evidenceResult.errors + documentsResult.errors,
          details: {
            cases: casesResult,
