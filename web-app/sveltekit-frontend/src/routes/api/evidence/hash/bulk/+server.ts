...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  if (!hashes && !evidenceIds) {
    return json({ error: 'Either hashes or evidenceIds array required' }, { status: 400 });
  }
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    const stats = {
      ...
      hashCoverage: recentEvidence.length > 0 
        ? ((recentEvidence.filter(e => e.hash).length / recentEvidence.length) * 100).toFixed(1)
        : '0',
