...
import { evidence, hashVerifications, users } from '$lib/server/db/schema';
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const { evidenceId, verifiedHash, method = 'manual', notes } = await request.json();
  ...
  try {
    ...
    const verificationResult = await db.insert(hashVerifications).values({
      evidenceId,
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    return json({ error: 'Failed to fetch verification history' }, { status: 500 });
  }
};
