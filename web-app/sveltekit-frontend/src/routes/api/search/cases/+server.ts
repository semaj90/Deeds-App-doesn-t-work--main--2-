...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    const cachedResults = await getCachedSearchResults(query, `cases_${searchType}`, filters);
    ...
    if (isPostgreSQL()) {
      await logSearchPerformance('case_search', searchType, executionTime, results.length);
    }
...
async function searchCasesText(query: string, limit: number, filters: any): Promise<any[]> {
  try {
    // Import the appropriate schema based on environment
    const { cases } = isPostgreSQL() 
      ? await import('$lib/server/db/unified-schema')
...
async function searchCasesSemantic(query: string, limit: number, filters: any): Promise<any[]> {
  try {
    ...
    const { cases } = await import('$lib/server/db/unified-schema');
    ...
    const results = await db
      ...
      .from(cases)
      .where(and(...whereConditions))
      .orderBy(sql`1 - (${cases.titleEmbedding} <=> ${JSON.stringify(embedding)}::vector) DESC`)
      .limit(limit);
...
async function searchCasesHybrid(query: string, limit: number, filters: any): Promise<any[]> {
  try {
    ...
    semanticResults.forEach(result => {
      if (combinedResults.has(result.id)) {
        ...
        combinedResults.set(result.id, {
          ...existing,
          searchScore: Math.max(existing.searchScore || 0, result.searchScore || 0) * 1.2,
          matchType: 'hybrid'
        });
      } else {
