  ...
  try {
    ...
    switch (searchMode) {
      case 'text':
        ...
        results = await searchEvidenceText(query, { caseId, evidenceType, limit });
        break;
        
      case 'content':
        // Deep content search using Qdrant
        results = await searchEvidenceContent(query, { caseId, evidenceType, limit });
        break;
        
      case 'semantic':
        // PostgreSQL vector search
        results = await searchEvidenceSemantic(query, { caseId, evidenceType, limit });
        break;
        
      case 'hybrid':
      default:
        // Best of all worlds
        results = await searchEvidenceHybrid(query, { caseId, evidenceType, limit });
        break;
    }
...
async function searchEvidenceHybrid(query: string, options: any) {
  ...
  const [textResults, contentResults, semanticResults] = await Promise.allSettled([
    ...
    searchEvidenceSemantic(query, { ...options, limit: Math.ceil(limit / 3) }),
  ]);
